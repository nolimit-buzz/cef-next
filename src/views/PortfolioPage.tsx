...
import type {
  ...
  PerformanceSection as PerformanceSectionData,
  ProjectsSection as ProjectsSectionData,
  NigeriaMapSection as NigeriaMapSectionData,
  ResolvedSdg,
  ResolvedProject,
...
export const PortfolioPage = ({ sections }: { sections: PortfolioPageSection[] }) => {
  const hero = sections.find(
    (s): s is HeroSectionData => s.__component === "portfolio-page.hero-section",
  );
  const performance = sections.find(
    (s): s is PerformanceSectionData => s.__component === "portfolio-page.performance-section",
  );
  const projectsSection = sections.find(
    (s): s is ProjectsSectionData => s.__component === "portfolio-page.projects-section",
  );
  const nigeriaMapSection = sections.find(
    (s): s is NigeriaMapSectionData => s.__component === "portfolio-page.nigeria-map-section",
  );

  const resolvedProjects = (projectsSection?.projects ?? []).map(resolveProject);
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      {projectsSection && (
        <ProjectsSection
          ...
          headingPrimary={projectsSection.headingPrimary}
          headingSecondary={projectsSection.headingSecondary}
          filterLabel={projectsSection.filterLabel}
          locationLabel={projectsSection.locationLabel}
          statusLabel={projectsSection.statusLabel}
          sdgImpactLabel={projectsSection.sdgImpactLabel}
          readMoreLabel={projectsSection.readMoreLabel}
          readLessLabel={projectsSection.readLessLabel}
          fullCaseStudyLabel={projectsSection.fullCaseStudyLabel}
          viewCaseStudyLabel={projectsSection.viewCaseStudyLabel}
          projects={resolvedProjects}
        />
      )}
      {nigeriaMapSection && (
        <NigeriaMap
          eyebrow={nigeriaMapSection.eyebrow}
          headingPrimary={nigeriaMapSection.headingPrimary}
          headingSecondary={nigeriaMapSection.headingSecondary}
          description={nigeriaMapSection.description}
          mapRegionLabel={nigeriaMapSection.mapRegionLabel}
          apiConnectedLabel={nigeriaMapSection.apiConnectedLabel}
          apiConnectedStatus={nigeriaMapSection.apiConnectedStatus}
          emptyStateHeading={nigeriaMapSection.emptyStateHeading}
          emptyStateBody={nigeriaMapSection.emptyStateBody}
          regionalImpactLabel={nigeriaMapSection.regionalImpactLabel}
          currentProjectsLabel={nigeriaMapSection.currentProjectsLabel}
          aggregatedSdgLabel={nigeriaMapSection.aggregatedSdgLabel}
...
const heroStatIcons = [Zap, Globe2, LayoutGrid];

const HeroSection = ({ badge, headingPrimary, headingSecondary, subHeadline, stats }: HeroSectionData) => {
  const { scrollY } = useScroll();
  const heroOpacity = useTransform(scrollY, [0, 600], [1, 0]);
  const accentStats = stats.slice(0, -1);
    ...
    <section className="relative bg-[var(--color-background)] flex flex-col border-b border-white/5 lg:h-[100dvh] lg:min-h-[700px] lg:max-h-[1080px]">
      ...
      <div className="w-full grid grid-cols-1 lg:grid-cols-4 z-20 relative shrink-0">
        ...
        <div className="bg-[#0094da] p-8 lg:py-12 xl:py-14 lg:px-10 flex flex-col justify-center gap-6 border-t border-white/10 relative overflow-hidden">
          ...
          {accentStats.map((stat, idx) => {
            const Icon = heroStatIcons[idx % heroStatIcons.length];
            return (
              <div key={stat.id}>
                <motion.div initial="hidden" animate="visible" variants={fadeUp} className="flex items-center gap-4 relative z-10">
                  ...
                  <div className="flex flex-col">
                    <span className="text-xl xl:text-2xl font-light text-white leading-snug">{stat.value}</span>
                    <span className="text-[10px] xl:text-xs font-bold text-white/70 uppercase tracking-widest">{stat.label}</span>
                  </div>
                </motion.div>
                {idx < accentStats.length - 1 && <div className="w-full h-px bg-white/20 relative z-10" />}
              </div>
            );
          })}
        </div>
        ...
        <div className="bg-[#32a99f] p-8 lg:py-12 xl:py-14 lg:pr-[max(1.5rem,calc((100vw-80rem)/2+1.5rem))] lg:pl-10 flex items-center border-t border-[#32a99f]">
          {lastStat && (
            <motion.div initial="hidden" animate="visible" variants={fadeUp} className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                <span className="text-xs xl:text-sm font-bold text-white/80 uppercase tracking-widest mb-1">{lastStat.label}</span>
                <span className="text-lg xl:text-xl font-bold text-white uppercase tracking-wide">{lastStat.value}</span>
              </div>
            </motion.div>
          )}
        </div>
...
const SdgBadge = ({ id, color, label }: ResolvedSdg) => (
  <div
  ...
  >
    <div
      ...
      style={{ backgroundColor: color, color: id === 7 ? 'black' : 'white' }}
    >
      {id}
    </div>
    <span className="text-[10px] font-medium" style={{ color }}>SDG {id}</span>
...
const PerformanceSection = ({
  ...
  const [activeTab, setActiveTab] = useState<'SDG Impact Highlights' | 'Financial Performance'>('SDG Impact Highlights');

  const tabs = [
    { key: 'SDG Impact Highlights' as const, label: tabSdgLabel },
    { key: 'Financial Performance' as const, label: tabFinancialLabel },
  ];
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="min-h-[400px]">
          <AnimatePresence mode="wait">
            {activeTab === 'SDG Impact Highlights' && (
              <motion.div
                key="sdg-highlights"
                initial={{ opacity: 0, y: 20 }}
                  ...
                  {sdgCards.map((card) => {
                    ...
                    return (
                      <motion.div
                        ...
                        className="group relative bg-white border border-blue-100/60 p-8 flex flex-col justify-between min-h-[240px] overflow-hidden transition-all duration-500 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-blue-200/60 hover:-translate-y-0.5"
                      >
                                ...
                                <div>
                                  <p className="text-xl md:text-2xl text-white/70 font-light leading-relaxed mb-12">
                                    ...
                                    {item.description.length > 120 && (
                                      ...
                                      >
                                        {expandedTextId === item.id ? readLessLabel : readMoreLabel}
                                      </button>
                                    )}
                                  </p>
                                ...
                                >
                                  <div className="flex items-center gap-6">
                                    ...
                                    <div className="flex flex-wrap items-center gap-2">
                                      {item.sdgs.map((sdg) => (
                                        <motion.div key={sdg.id} variants={itemVariants}>
                                          <SdgBadge {...sdg} />
                                        </motion.div>
                                      ))}
                                    </div>
